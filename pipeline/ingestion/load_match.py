...
def get_exact_source_file_id(
    ...
    rows = result.data or []
    if not rows:
        log.warning(f"  No registered source_file for {slug} / {source_type}; provenance left blank")
        return None
    if len(rows) > 1:
        paths = ", ".join(str(row.get("storage_path", "")) for row in rows)
        log.warning(
            f"  Multiple active source_file rows for {slug} / {source_type}: {paths}; "
...
def load_match(slug: str, season: str, dry_run: bool = False):
    ...
    if players_df is not None:
        ...
        source_file_id = get_exact_source_file_id(sb, season, slug, "sportscode")
